class Solution:
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        ...
        border_indices_pacific = {(r,c) for r in range(ROW_LEN) for c  in range(COL_LEN) if r == 0 or c == 0}
        border_indices_atlantic = {(r,c) for r in range(ROW_LEN) for c  in range(COL_LEN) if r == ROW_LEN-1 or c == COL_LEN-1}
